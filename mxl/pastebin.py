...
class PasteBin:
	...
	async def api_call(self, method, params):
		async with aiohttp.request('POST', 'https://pastebin.com/api/' + method, data=params, timeout=self.timeout) as response:
			print(response.url)
			return await response.text()

	async def create_user_key(self, username, password):
		params = {'api_dev_key':self.api_dev_key, 'api_user_name':username, 'api_user_password':password}
		return await self.api_call('api_login.php', params)

	async def paste(self, data, guest=False, name=None, format=None, private=None, expire=None):
		...
		return await self.api_call('api_post.php', params)

	async def list_pastes(self, results_limit=None):
	...
	async def trending_pastes(self):
